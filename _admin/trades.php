...
if (isset($_POST['_form_submit'])&&$_POST['_add_sell']==1){
	...
	if ($num_rows>0){
		...
		if ($row['trade_type']==1){
			...
			addLog('Back-end','Option Trades',''.$uDetails['user_firstname'].' '.$uDetails['user_lastname'].' ('.$uDetails['user_account_num'].')',''.$_SESSION['admin']['name'].' ('.$_SESSION['admin']['refnum'].')','Sell added '.($tradeRef+0).' ('.$tradesSellStatuses[$_POST['trade_status']].')');
		}else{
			$mysql_fields='';
			...
			$uDetails=$db->getRow('users','user_account_num="'.$_POST['user_account_num'].'"','user_firstname, user_lastname, user_account_num');
			addLog('Back-end','Option Trades',''.$uDetails['user_firstname'].' '.$uDetails['user_lastname'].' ('.$uDetails['user_account_num'].')',''.$_SESSION['admin']['name'].' ('.$_SESSION['admin']['refnum'].')','Sell edited '.($row['trade_ref']+0).' ('.$tradesSellStatuses[$_POST['trade_status']].')');
		}
	}
	$db->close();
...
if (isset($_POST['_form_submit'])&&$_POST['_add_edit_buy']==1){
	...
	if (!$_POST['tref']){
		...
		$uDetails=$db->getRow('users','user_account_num="'.$_POST['user_account_num'].'"','user_firstname, user_lastname, user_account_num');
		addLog('Back-end','Option Trades',''.$uDetails['user_firstname'].' '.$uDetails['user_lastname'].' ('.$uDetails['user_account_num'].')',''.$_SESSION['admin']['name'].' ('.$_SESSION['admin']['refnum'].')','Buy added '.($tradeRef+0).' ('.$tradesBuyOptions[$_POST['trade_option']].' @ '.$tradesStatuses[$_POST['trade_status']].')');
	}else{
		...
		$uDetails=$db->getRow('users','user_account_num="'.$_POST['user_account_num'].'"','user_firstname, user_lastname, user_account_num');
		addLog('Back-end','Option Trades',''.$uDetails['user_firstname'].' '.$uDetails['user_lastname'].' ('.$uDetails['user_account_num'].')',''.$_SESSION['admin']['name'].' ('.$_SESSION['admin']['refnum'].')','Buy edited '.($_POST['tref']+0).' ('.$tradesBuyOptions[$_POST['trade_option']].' @ '.$tradesStatuses[$_POST['trade_status']].')');
	}
...
jQuery(document).ready(
	function($) {
		...
		$("#sliderVal").keyup(function() {
			...
			if (sliderValue >= 1 && sliderValue <= 9999) {
...
<div class="left">
	...
	foreach ($tradesBuyOptions as $OptionID=>$OptionName){
		$pcontent.='<option value="'.$OptionID.'"'.(($OptionID==array_get($_POST, 'trade_option'))?' selected':'').'>'.$OptionName.'</option>';
	}
	...
	$res=$db->rq($query);
	while (($row=$db->fetch($res))!=FALSE){
		$pcontent.='<option value="'.$row['commodities_id'].'_'.$row['commodities_symbol'].'_'.$row['commodities_contract_size'].'_'.$row['commodities_unit'].'_'.$row['commodities_def_fee'].'_'.$row['commodities_def_prem'].'"'.(($row['commodities_id']==array_get($_POST, 'commodities_id'))?' selected':'').'>'.$row['commodities_symbol'].' ('.$row['commodities_name'].')</option>';
	}
	...
	while (($row=$db->fetch($res))!=FALSE){
		...
		$pcontent.='<option value="'.$row['expiry_date'].'"'.(($row['expiry_date']==array_get($_POST, 'trade_expiry_date'))?' selected':'').'>'.$row['expiry_short'].' ('.date('d M y', $strToUse).')</option>';
	}
	...
	<div class="formsRight">
		<input class="text-input align-right" type="text" name="trade_strikeprice" id="trade_strikeprice" value="'.array_get($_POST, 'trade_strikeprice').'"'.$JSCripts.' />
	</div><br />
	...
	<input class="ui-state-default trade-details" type="text" name="trade_details" id="trade_details" value="'.array_get($_POST, 'trade_details', 'BUY').'" readonly />
	</fieldset>';
	
	$totalRelated=0;
	$query2='SELECT trade_details, trades.trade_ref FROM trades_related tr LEFT JOIN trades ON tr.trade_ref=trades.trade_ref WHERE trade_ref_relatedto="'.array_get($_POST, 'trade_ref').'"';
	$res2=$db->rq($query2);
	...
	if ($totalRelated>0){
		...
		while (($row2=$db->fetch($res2))!=FALSE){
			$pcontent.='<div class="ui-state-default trade-details"><a href="trades.php?action=edit_sell&tref='.$row2['trade_ref'].'" style="display:block;">'.$row2['trade_details'].'</a></div><br />';
		}
...
<div class="left">
	...
	<div class="formsRight">
		<input class="text-input align-right" type="text" name="trade_premium_price" id="trade_premium_price" value="'.array_get($_POST, 'trade_premium_price').'"'.$JSCriptsPremium.' />
	</div>
	<br />
	<div class="formsLeft">Contract Size:</div>
	<div class="formsRight">
		<input class="text-input align-right ui-state-default" type="text" name="trade_contract_size" id="trade_contract_size" value="'.array_get($_POST, 'trade_contract_size').'" readonly />
	</div>
	<br />
	<div class="formsLeft">Price/contract:</div>
	<div class="formsRight">
		<input class="text-input align-right" type="text" name="trade_price_contract" id="trade_price_contract" value="'.array_get($_POST, 'trade_price_contract').'"'.$JSCripts.' />
	</div>
	<br />
	...
	<div class="formsRight"><input class="text-input" type="text" name="trade_date" id="trade_date" value="'.array_get($_POST, 'trade_date').'" /></div>
	<br />
...
switch ($cmd) {
	...
	case 'delete_buy' :
		if ($_SESSION['admin']['is_logged']==1){
			...
			global $tradesStatuses;
			addLog('Back-end','Option Trades',''.$uDetails['user_firstname'].' '.$uDetails['user_lastname'].' ('.$uDetails['user_account_num'].')',''.$_SESSION['admin']['name'].' ('.$_SESSION['admin']['refnum'].')','Buy deleted '.($row['trade_ref']+0).' ('.$tradesStatuses[$row['trade_status']].')');
	...
	case 'delete_sell' :
		if ($_SESSION['admin']['is_logged']==1){
			...
			$uDetails=$db->getRow('users','user_account_num="'.$row['user_account_num'].'"','user_firstname, user_lastname, user_account_num');
			global $tradesSellStatuses;
			addLog('Back-end','Option Trades',''.$uDetails['user_firstname'].' '.$uDetails['user_lastname'].' ('.$uDetails['user_account_num'].')',''.$_SESSION['admin']['name'].' ('.$_SESSION['admin']['refnum'].')','Sell deleted '.($row['trade_ref']+0).' ('.$tradesSellStatuses[$row['trade_status']].')');
