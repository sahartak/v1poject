...
function addNewDeposit($transferID=0) {
	...
	if (array_get($_POST, 'tr_date', '') == ''){
        $_POST['tr_date']=date('Y-m-d', CUSTOMTIME);
    }
	if (array_get($_POST, 'tr_fees', '') == ''){
        $_POST['tr_fees']='0.00';
    }
...
<div class="left">
	...
	<input type="button" name="_delete" value="'.getLang('sform_delbtn').'" class="submitBtn ui-state-default" onclick="if(confirm(\'Are you sure you want to delete this deposit?\')) location=\'?action=delete_deposit&dtrid='.(array_get($_POST, 'tr_ref')).'\';" />';
	}
	$pcontent.='
	<input type="button" name="_cancel" value="'.getLang('sform_backbtn').'" class="submitBtn ui-state-default" onclick="'.((array_get($_GET, 'uid')>0)?'location=\'users.php\'':'location=\'transfers.php\'').';" />
	</div>
</div>
</form>
</div>';
