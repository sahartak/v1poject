...
if ($num_rows>0){
	...
	if ($row['trade_type']==4){
		...
		$mysql_fields.='trade_shares="'.($_POST['trade_shares']+$row['trades_shares']).'", ';
		
		$NewSharesCount=$row['trade_shares_left']-$_POST['trade_shares'];
		$mysql_fields.='trade_shares_left="'.$NewSharesCount.'", ';
		...
		$query='INSERT INTO stock_trades SET '.$mysql_fields.', trade_type=3, trades_id="'.NID.'", trade_ref='.($tradeRef+0); //.', trade_date="'.date('Y-m-d H:i:s', CUSTOMTIME).'"'
		$db->rq($query);
		...
		addLog('Back-end','Stock Trades',''.$uDetails['user_firstname'].' '.$uDetails['user_lastname'].' ('.$uDetails['user_account_num'].')',''.$_SESSION['admin']['name'].' ('.$_SESSION['admin']['refnum'].')','Cover added '.($tradeRef+0).' ('.$tradesSellStatuses[$_POST['trade_status']].')');
	}else{
		//EDIT COVER ORDER NOT YET DONE
		$mysql_fields='';
