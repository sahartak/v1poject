...
function addNewTradeBuy($tradesBuy_id=0) {
	...
	$db=new DBConnection();
	if ($tradesBuy_id && !isset($_POST['_form_submit'])){
		$query='SELECT * FROM stock_trades WHERE trade_ref="'.$tradesBuy_id.'"';
		$res=$db->rq($query);
		$_POST=$db->fetch($res);
...
<script type="text/javascript" src="js/forms/stradesBuy.js"></script>
<script type="text/javascript">
jQuery(document).ready(
	function($) {
		var sliderValue = '.array_get($_POST, 'trade_shares', 10).';
		$("#slider").slider( {
			min : 1,
			max : 999999,
			step : 1,
		...
		$("#sliderVal").keyup(function() {
			...
			if (sliderValue >= 1 && sliderValue <= 999999) {
				$("#slider").slider("value", sliderValue);
			} else {
				alert("Please enter a value between 1 and 999999");
...
<div class="left">
	...
	<input class="ui-state-default trade-details" type="text" name="trade_details" id="trade_details" value="'.((array_get($_POST, 'trade_details') != '')?''.$_POST['trade_details'].'':'BUY').'" readonly />
	</fieldset>';
...
<div class="left">
	...
	<div class="formsRight">
		<input class="text-input align-right" type="text" name="trade_price_share" id="trade_price_share" value="'.array_get($_POST, 'trade_price_share').'"'.$JSCriptsPremium.' />
	</div>
	<br />
