...
$config = App\Utility\Config::getConfig('database');
...
define('UL_PDO_CON_STRING', 'mysql:host=' . $config['host'] . ';dbname=' . $config['dbname']);
// Example for SQLite: 
...
// logins table.
define('UL_PDO_AUTH_USER', $config['username']);
define('UL_PDO_AUTH_PWD', $config['password']);

// LOGIN UPDATE
// Used to add new and modify login data.
// Database user with SELECT, UPDATE and INSERT access to the
// logins table.
define('UL_PDO_UPDATE_USER', $config['username']);
define('UL_PDO_UPDATE_PWD', $config['password']);

// LOGIN DELETE
// Used to remove logins.
// Database user with SELECT and DELETE access to the
// logins table
define('UL_PDO_DELETE_USER', $config['username']);
define('UL_PDO_DELETE_PWD', $config['password']);

// SESSION
// Database user with SELECT, UPDATE and DELETE permissions to the
// sessions and nonces tables.
define('UL_PDO_SESSIONS_USER', $config['username']);
define('UL_PDO_SESSIONS_PWD', $config['password']);

// LOG
// Used to log events and analyze previous activity.
// Database user with SELECT, INSERT and DELETE access to the
// logins-log table.
define('UL_PDO_LOG_USER', $config['username']);
define('UL_PDO_LOG_PWD', $config['password']);
