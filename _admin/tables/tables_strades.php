...
$searchQuery='SELECT SQL_CALC_FOUND_ROWS * FROM stock_trades LEFT JOIN users ON stock_trades.user_account_num=users.user_account_num '.$searchFor.' '.$searchOrder.' '.$searchLimit.'';
...
while (($aRow=$db->fetch($rResult))!=FALSE) {
    ...
    if($aRow['trade_status']=='Open') {
    	$sOutput.=addslashes('<a href="strades.php?action=new_sell&tref='.$aRow['trade_ref'].'"><i class="fa fa-exchange fa-lg" title="SELL out this BUY"></i></a>');
    }
    $sOutput.=' ';
    ...
    $sOutput.=' ';
    $sOutput.=addslashes('<a href="strades.php?action=pdf&ref='.($aRow['trade_ref']).'"><i class="fa fa-file-text fa-lg" title="Generate PDF"></i></a>');
    $sOutput.='"';
    $sOutput.="],";
}
