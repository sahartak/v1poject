...
$searchQuery='SELECT SQL_CALC_FOUND_ROWS * FROM trades LEFT JOIN users ON trades.user_account_num=users.user_account_num 
...
while (($aRow=$db->fetch($rResult))!=FALSE) {
    ...
    if($aRow['trade_status']=='Open') {
    	$sOutput.=addslashes('<a href="trades.php?action=new_sell&tref='.$aRow['trade_ref'].'"><img src="images/trades_sell.png" title="Add SELL from this BUY" border="0"></a>');
    }else{
    	$sOutput.='-';
    }
    $sOutput.='"';
