	...
	$.fn.dataTable = function( oInit )
	{
		...
		function _fnAjaxUpdate( oSettings )
		{
			if ( oSettings.bAjaxDataGet )
			{
				...
				if ( oSettings.oFeatures.bFilter !== false )
				{
					...
					for ( i=0 ; i<iColumns ; i++ )
					{
						if(oSettings.sSearchIDs[i]==0) {
							aoData.push( { "name": "sSearch_"+i,      "value": oSettings.aoPreSearchCols[i].sSearch } );
							aoData.push( { "name": "bEscapeRegex_"+i, "value": oSettings.aoPreSearchCols[i].bEscapeRegex } );
						}else{
							aoData.push( { "name": "sSearch_"+oSettings.sSearchIDs[i],      "value": oSettings.aoPreSearchCols[i].sSearch } );
							aoData.push( { "name": "bEscapeRegex_"+oSettings.sSearchIDs[i], "value": oSettings.aoPreSearchCols[i].bEscapeRegex } );
						}
					}
				}
		...
		return this.each(function()
		{
			...
			if ( typeof oInit != 'undefined' && oInit !== null )
			{
				...
				_fnMap( oSettings, oInit, "sSearchIDs" );
