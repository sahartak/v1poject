...
$(document).ready(function() {
    var subSearchType=$("#subSearchType").val();
	...
	"fnDrawCallback": function( oSettings ) {
	    $('#usersTable tbody tr a').click( function(e) {
		aPos = oTable.fnGetPosition(this.parentNode);
		Data = oSettings.aoData[aPos[0]]._aData[0];
		GetID=$(this).attr("id");
		link=$(this).attr("href");
		if(GetID!="NewWindow") {
		...
		Data = oSettings.aoData[iPos]._aData[0];
		e.preventDefault();
		var src = "../dosuuser.php?uid="+ Data;
		$.modal('<iframe src="' + src + '" height="650" width="830" style="border:0" scrolling=auto>', {
		    containerCss:{
			backgroundColor:"#fff",
			borderColor:"#999999",
			height:650,
	...
	"bServerSide": true,
	"bAutoWidth": false,
	"sAjaxSource": "tables/tables_users.php",
	"bJQueryUI": true,
	"bSortClasses": false,
	"sPaginationType": "full_numbers",
	"fnServerData": function ( sSource, aoData, fnCallback ) {
	    /* Add some extra data to the sender */
	    aoData.push({"name": "subSearch","value": subSearchType});
	    $.getJSON( sSource, aoData, function (json) {
    ...
    $("#usersTable tbody").mouseover(function(event) {
