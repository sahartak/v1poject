...
WHERE trade_type=4 AND trade_status=1'.$searchFor.' '.$searchOrder.' '.$searchLimit.'';
...
WHERE trade_type=4 AND trade_status=1';
$res=$db->rq($query);
$row=$db->fetch($res);
...
while (($aRow=$db->fetch($rResult))!=FALSE) {
    ...
    if($aRow['trade_status']=='Open') {
        $sOutput.=addslashes('<a href="strades.php?action=new_cover&tref='.$aRow['trade_ref'].'"><img src="images/trades_sell.png" title="Add SELL from this BUY" border="0"></a>');
    }else{
        $sOutput.='-';
    }
    $sOutput.='"';
